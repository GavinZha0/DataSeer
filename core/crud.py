...
class DalBase:
    ...
    async def get_data(
        ...
        :param v_ret: MODEL: orm model object(default); SCHEMA: schema object; DUMP: serialized schema (dict)
        ...
        if not isinstance(v_start_sql, SelectType):
            if hasattr(self.model, 'deleted'):
                v_start_sql = select(self.model).where(self.model.deleted == false())
            elif hasattr(self.model, 'is_delete'):
        ...
        if data and v_ret:
            if v_ret == RET.SCHEMA:
                # validate data and map to schema object
                return self.schema.model_validate(data)
            elif v_ret == RET.DUMP:
                # map to schema and serialize it to json dict
    ...
    async def get_datas(
        ...
        :param v_order: 排序，默认正序，为 desc 是倒叙
        :param v_order_field: 排序字段
        :param v_count: 默认为 False，是否返回 count 过滤后的数据总数，不会影响其他返回结果，会一起返回为一个数组
        :param v_ret: MODEL: orm model object(default); SCHEMA: schema object; DUMP: serialized schema (dict)
        ...
        if v_ret == RET.SCHEMA:
            datas = [self.schema.model_validate(i) for i in result]
        elif v_ret == RET.DUMP:
            datas = [self.schema.model_validate(i).model_dump() for i in result]
        else:
            datas = list(result)
    ...
    async def delete_data(self, data_id: int, v_soft: bool = False, **kwargs) -> None:
        """
        删除多条数据
        :param data_id: id
        :param v_soft: 是否执行软删除
        ...
        if v_soft:
            await self.db.execute(
                update(self.model).where(self.model.id.in_(data_id)).values(
                    delete_datetime=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    is_delete=True,
                    **kwargs
                )
            ...
            await self.db.execute(delete(self.model).where(self.model.id.in_(data_id)))
        await self.flush()
                ...
                if isinstance(value, tuple):
                    if len(value) == 1:
                        if value[0] == "None":
                            ...
                            raise CustomException("SQL syntax error")
